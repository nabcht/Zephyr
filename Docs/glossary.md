...
This glossary covers both project-specific terminology used inside **Zephyr** and the broader AI, runtime, and web terms that show up across the codebase, docs, and Control Room.

## Zephyr Product Terms

- **Zephyr:** A local-first AI sidekick built around a shared Python runtime and a browser-based operator surface.
- **Control Room:** The primary React web interface for chat, missions, runtime visibility, and operator workflows.
- **Chat Workspace:** The main browser conversation surface for chat, missions, slash commands, and session attachments.
...
- **Support Page:** The in-shell recovery and navigation page for common operator troubleshooting paths.
...
- **Slash Commands:** Browser or CLI commands such as `/help`, `/reload`, or `/mission <task>` that map onto inspection or runtime actions.
...
- **Runtime Gateway:** The backend helper layer that exposes shared runtime instances and lazy initialization helpers to services.
...
- **Runtime Activity Stream:** The SSE log emitted by reload and prepare actions and rendered in the Activity page.
...
- **Session Attachment:** A file uploaded in the browser, stored under `temp_core/attachments`, indexed locally, and scoped to one web session for retrieval.
...
- **Inference Backend:** The actual model-serving backend used by the runtime. Zephyr currently supports Ollama, LlamaCPP, and OpenRouter.
...
- **Inference Metrics:** Warm-up, first-token, and completion timing values exposed in system status and rendered in the Activity page.
- **Provider Payload Metrics:** Observability fields that report message counts, tool-schema counts, and serialized payload size for the most recent provider call.
- **Safety Confirmation:** The per-request approval path that controls whether sensitive tools are allowed in browser chat or mission turns.
...
## General Development Terms Used In Zephyr
